...
class GCN:
    def __init__(self, sizes, An, X_obs, name="", with_relu=True, params_dict={'dropout': 0.5}, gpu_id=0,
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope(name) as scope:
                ...
                self.node_labels = tf.placeholder(tf.int32, [None, sizes[2]], 'node_labels')
                ...
                #self.An = tf.SparseTensorValue(np.array(An.nonzero()).T, np.array(An[An.nonzero()].A1, dtype=np.float32), An.shape)
                ...
                self.h2 = spdot(self.An, dot(self.h1_comp, self.W2))
                if with_relu:
                    self.h2 = tf.nn.relu(self.h2 + self.b2)
                self.h2_dropout = tf.nn.dropout(self.h2, 1 - self.dropout)


                self.h2_comp = tf.cond(self.training,
                                       lambda: self.h2_dropout,
                                       lambda: self.h2) if self.dropout > 0. else self.h2


                self.W3 = slim.variable('W3', [sizes[1], sizes[2]], tf.float32, initializer=w_init())
                self.b3 = slim.variable('b3', dtype=tf.float32, initializer=tf.zeros(sizes[2]))
                
                self.logits = spdot(self.An, dot(self.h2_comp, self.W3))


                if with_relu:
                    self.logits += self.b3
                self.logits_gather = tf.gather(self.logits, self.node_ids) #拿出对应需要位置的预测概率
                ...
                var_l = [self.W1, self.W2, self.W3]
                if with_relu:
                    var_l.extend([self.b1, self.b2, self.b3])
    ...
    def convert_varname(self, vname, to_namespace=None):
        """
        Utility function that converts variable names to the input namespace.替换成之前设定的name="surrogate"
    ...
    def set_variables(self, var_dict):
        ...
        after training with patience.#存储最优参数，并在训练结束之后把参数返回到模型中去，与存储在本地的checkpoint类似但是更快一些
    ...
    def train(self, An, split_train, split_val, Z_obs, patience=30, n_iters=400, print_info=True):
        """
        ...
        for it in range(n_iters):
            ...
            # print ( 'The performance is f1_micro: {0}, f1_macro: {1} and perf_sum: {2} in {3} iteration'.format ( f1_micro, f1_macro, perf_sum, it ) )
            acc_val = eval_class_acc_noretrain(split_val, self, np.argmax(Z_obs, 1), An)
            perf_val = acc_val
            print ( 'The validation accuracy performance is: {0} in {1} iteration'.format ( perf_val, it ) )

            # acc_test = eval_class_acc ( split_test, self, np.argmax ( Z_obs, 1 ) )
            # perf_test = acc_test
            # print ( 'The test accuracy performance is: {0} in {1} iteration'.format ( perf_test, it ) )
    ...
    #def test(self, split_test, Z_obs):
    def test(self, split_test, Z_obs, adj):

        # feed = {self.node_ids: split_test,
        #         self.node_labels: Z_obs[split_test]}
        # pred = self.session.run(self.predictions, feed)
        acc_test = eval_class_acc_noretrain ( split_test, self, np.argmax ( Z_obs, 1 ), adj )
        perf_test = acc_test
...
def eval_class_acc(ids_to_eval, model, z_obs):
    """
    Evaluate the model's classification performance.
...
def eval_class_acc_noretrain(ids_to_eval, model, z_obs, An):
