...
class vaegan(object):
    ...
    def __init__(self, batch_size, max_iters, repeat, model_path, data_ob, latent_dim, sample_path, log_dir, learnrate_init):
        ...
        self.dataset = tf.data.Dataset.from_tensor_slices(
            tf.convert_to_tensor(self.data_ob.train_data_list, dtype=tf.string))
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            while step <= self.max_iters:
                ...
                next_x_images = next_x_images[:,:,:,np.newaxis]
    ...
    def generate(self, z_var, reuse=False):
        ...
        with tf.variable_scope('generator') as scope:
            ...
            d2 = tf.nn.relu(batch_normal(de_conv(d2 , output_shape=[self.batch_size, 7,7, 256], name='gen_deconv2'), scope='gen_bn2', reuse=reuse))
            d3 = tf.nn.relu(batch_normal(de_conv(d2, output_shape=[self.batch_size, 14, 14, 128], name='gen_deconv3'), scope='gen_bn3', reuse=reuse))
            d4 = tf.nn.relu(batch_normal(de_conv(d3, output_shape=[self.batch_size, 28, 28, 32], name='gen_deconv4'), scope='gen_bn4', reuse=reuse))
            #d5 = de_conv(d4, output_shape=[self.batch_size, 64, 64, 1], name='gen_deconv5', d_h=1, d_w=1)
            d5 = de_conv(d4, output_shape=[self.batch_size, 28, 28, 1], name='gen_deconv5', d_h=1, d_w=1)
    ...
    def Encode(self, x):
        ...
        with tf.variable_scope('encode') as scope:
            ...
            conv3 = tf.reshape(conv3, [self.batch_size, 256 * 4 * 4])   # reshape error
    ...
    def _parse_function(self, images_filenames):
        ...
        image_decoded = tf.image.decode_and_crop_jpeg(image_string, crop_window=[218 / 2 - 54, 178 / 2 - 54 , 108, 108], channels=self.channel)
    ...
    def _read_by_function_old(self, filename):
